...
const Sidebar = () => {
    ...
    const fetchCategories = async () => {
        try {
            const res = await axiosInstance.get("/product/categories"); // This endpoint should return populated subCategories
            ...
            <ul className="space-y-3">
                ...
                {categories.map((category) => (
                    <li key={category._id} className="ml-1">
                        <p className="font-normal text-lg mb-2 text-gray-700 cursor-pointer hover:text-black transition-colors">
                            {category.name}
                        </p>
